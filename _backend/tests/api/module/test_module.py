...
def test_post_module_uni_admin_success(mocker):
    load_env()
    init_setting()
    class InsertedCursor(BaseModel):
        inserted_ids: list
    mocker.patch('app.crud.modules.insert_many', return_value=InsertedCursor(inserted_ids=["65ac17b1d2815b505f3e352d", "65ac17b1d2815b505f3e352e"]))
    mocker.patch('fastapi.BackgroundTasks.add_task', return_value=True)
    response = client.post(
        url=f'/api/v1/module',
        headers={"Content-Type":"application/xml", "Authorization": f"Bearer {uni_admin_jwt}"},
        data='''<?xml version="1.0" encoding="UTF-8"?>
    ...
    expected_response = {
        'data': {
            ...
            'items': [
                {
                    'module_id': '65ac17b1d2815b505f3e352d', 
                    'module_name': 'Artificial Intelligence for Data Analysis', 
                    'degree_program': 'Engineering and Management', 
                    'degree_level': 'Master', 
                    'university': 'Technische Universitat Chemnitz', 
                    'module_code': '2GI018n', 
...
def test_post_module_sys_admin_success(mocker):
    load_env()
    init_setting()
    class InsertedCursor(BaseModel):
        inserted_ids: list
    ...
    response = client.post(
        url=f'/api/v1/module',
        headers={"Content-Type":"application/xml", "Authorization": f"Bearer {sys_admin_jwt}"},
        data='''<?xml version="1.0" encoding="UTF-8"?>
<data>
    <module>
        <module_name>Artificial Intelligence for Data Analysis</module_name>
